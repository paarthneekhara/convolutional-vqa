    ...
    def build_model(self, train = True):
        ...
        if options['words_vectors_provided']:
            source_sentence = tf.placeholder('float32', 
                [options['batch_size'],options['text_length'], options['length_of_word_vector']], 
                name = 'sentence')
            source_sentence_reshaped = tf.reshape(source_sentence,[-1,options['length_of_word_vector']])
            source_embedding = tf.nn.relu( ops.fully_connected(source_sentence_reshaped, 
                2*options['residual_channels'], name = "tm_source_embedding") )
            source_embedding = tf.reshape(source_embedding,[options['batch_size'],options['text_length'],-1])
            print "word vectors used, source sentence", source_embedding
        else:
